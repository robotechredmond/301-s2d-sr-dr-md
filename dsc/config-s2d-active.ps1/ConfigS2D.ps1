...
configuration ConfigS2D
{
    ...
    Import-DscResource -ModuleName xDisk, cDisk, xComputerManagement, xFailOverCluster, xActiveDirectory, xSOFS, xSR
    ...
    Node localhost
    {
        ...
        xWaitForFileShare WaitForRemoteShare
        {
        ...
        xSRAccess GrantSRAccessToRemoteCluster
        {
            ClusterName = $ClusterName
            RemoteClusterName = $RemoteClusterName
            DomainAdministratorCredential = $DomainCreds
            DependsOn = "[xWaitForFileShare]WaitForRemoteShare"
        }

        xSRPartnership CreateSRPartnership
        {
            ClusterName = $ClusterName
            RemoteClusterName = $RemoteClusterName
